...
def preprocessForm(filename, formsFolderName):
    ...
    filename = re.match(r"" + formsFolderName + "/(.*)\.png", filename).group(1)
...
def getFeaturesVectors(extractedLines, windowWidth):
    ...
    for index, img in enumerate(extractedLines):
        ...
        for i in range(w-windowWidth):
            ...
            if len(slidingWindow[slidingWindow == 255]) != 0:
                ...
                if gradient[0][topContour[1]][topContour[0]] != 0:
                    upperContourOrientation.append(math.atan(gradient[1][topContour[1]][topContour[0]]/gradient[0][topContour[1]][topContour[0]]))
                else:
                ...
                if gradient[0][bottomContour[1]][bottomContour[0]] != 0:
                    lowerContourOrientation.append(math.atan(gradient[1][bottomContour[1]][bottomContour[0]]/gradient[0][bottomContour[1]][bottomContour[0]]))
                else:
